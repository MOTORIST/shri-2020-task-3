...
export function activate(context: ExtensionContext) {
  ...
  schemaResolutionErrorStatusBarItem.tooltip =
    localize('json.schemaResolutionErrorMessage', 'Unable to resolve schema.') +
    ' ' +
  ...
  client.onReady().then(() => {
    ...
    toDispose.push(
      workspace.onDidChangeTextDocument(e => handleContentChange(e.document.uri)),
    );
    toDispose.push(
      ...
      toDispose.push(
        languages.registerSelectionRangeProvider(selector, {
          async provideSelectionRanges(
            ...
            const rawResult = await client.sendRequest<SelectionRange[][]>(
              '$/textDocument/selectionRanges',
              {
                textDocument,
                positions: positions.map(client.code2ProtocolConverter.asPosition),
              },
            );
            if (Array.isArray(rawResult)) {
              return rawResult.map(rawSelectionRanges => {
                return rawSelectionRanges.reduceRight(
                  (
...
function getSettings(): Settings {
  ...
  let schemaSettingsById: { [schemaId: string]: JSONSchemaSettings } = Object.create(
    null,
  );
